...
router.get('/', function(req, res, next) {
  res.render('index', { title: '(<ゝω·)Kira☆~', sourcePort: 8888, page: "video"});
});
...
router.get('/getFunImages', function (req, res, next) {
  var data = JSON.parse(fs.readFileSync(path.join(__dirname, '/static/fun_images.json'),'utf8')).fix;
...
router.get('/getRank', function (req, res, next) {
  ...
  var data = JSON.parse(fs.readFileSync(path.join(__dirname, './static/rank/r'+field+'-3.json'),'utf8'));
  res.send(data);
  res.end();
})
